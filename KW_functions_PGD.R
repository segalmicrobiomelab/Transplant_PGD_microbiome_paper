...
#turn dataframe into txt delimited with the same name
df_to_txt<- function(df){
  dataframe_txt<-paste0(df,".txt")
  write.table(get(df), dataframe_txt, sep = "\t", row.names = TRUE, col.names=FALSE)
}
...
beta_diversity_KW <- function(input_phyloseq, 
  ...
  pvalues<-round(x$`Pr(>F)`[[1]], digits=4)
...
boxplot_nonPS_KW <- function(input_table, 
                             ...
                             graph_option=c("boxplot","mean_SE", "mean_SD"), 
                             display_rank=c("yes","no"),
                             highlight_row=list(), #row that you want to highlight
                             output_prefix) {
  ...
  if (!missing(highlight_row)) {
    stopifnot("input into highlight_row has to be one of the rows in input_table "= highlight_row %in% input_table[[1]])
  } 
  
  #adding default setting
  if (missing(graph_option)) { graph_option<-"boxplot"} #if graph_option is missing, then will just have boxplot as default
  if (missing(log_scale)) { log_scale<-"no"} #if graph_option is missing, then will just have no as default
  if (missing(display_rank)) { display_rank<-"no"} #if display_rank is missing, then will just have no as default
  
  #making sure the input is within allowed 
  compare_stat_option <- match.arg(compare_stat_option)
  log_scale <- match.arg(log_scale)
  graph_option <- match.arg(graph_option)
  display_rank <- match.arg(display_rank)
  ...
  for (rank_sample_type in sample_types) {
    ...
    # Filtering ONLY the top 100 taxa 
    subset.top <- input_table5[rownames(input_table5) %in% rownames(top.3),]
    ...
    temp <- merge(temp, top.3, by.x="id", by.y="row.names")
    #changing the name of the y axis to add the rank order on it
    temp$id2<- temp[[paste0(paste0("rank.order.",rank_sample_type),".name")]]
    temp$id2 <- ifelse(nchar(temp$id2) > 25, 
                       paste0(substr(temp$id2, 1, 20), "....", substr(temp$id2, nchar(temp$id2) - 5, nchar(temp$id2))), 
                       temp$id2)
    if (!missing(highlight_row)) {
      highlight_row<-data.frame(highlight_row)
      highlight_row$highlight<-"TRUE"
      temp2 <- merge(temp, highlight_row, by.x =1, by.y=1, all.y = TRUE)
      temp2 <- temp2 %>% select(paste0("rank.order.",rank_sample_type))
      temp2<-na.omit(temp2)
      list_row_keep<-unique(temp2)
      # Create a character vector of length 100
      row_color_col <- rep("black", 100)
      # Set values corresponding to the positions of 3 and 4 to "RED"
      row_color_col[as.numeric(list_row_keep[[1]])] <- "red"
      row_color_col<-rev(row_color_col)
    } else {
      row_color_col<-"black" 
    }
    ...
    if (display_rank=="no"){
          p <- temp %>%   mutate(id = fct_reorder(id, -get(paste0("rank.order.",rank_sample_type)))) %>%
            ...
            coord_flip() +
            theme_bw() + 
            theme(axis.text.y=element_text(color=row_color_col,face="bold",size=10),axis.title.y=element_blank()) +
            theme(axis.text.x=element_text(angle=90,hjust=1))+
            ggtitle(rank_sample_type) 
    } else {
      p <- temp %>%   mutate(id2 = fct_reorder(id2, -get(paste0("rank.order.",rank_sample_type)))) %>%
...
beta_diversity_nonPS_KW <- function(input_table, 
                                    ...
                                    sample_types=list(), 
                                    sample_type_color=list(), 
                                    dot_sample_type_var_name,
                                    dot_sample_types=list(), 
                                    dot_sample_type_color=list(), 
                                    p_value=c("yes","no"),
                                    turn_relative_ab=c("yes","no"),
                                    output_name, 
  ...
  # Enforce orientation. Samples are columns
  if(missing(x_axis_flip)) { x_axis_flip<-"no"} #default being no for x_axis_flip
  if(missing(turn_relative_ab)) { turn_relative_ab<-"no"} #default being no for turn_relative_ab
  if(missing(p_value_location) & tolower(p_value)=="yes"){
    p_value_location<-"BL" #default location for p_value will be in the bottom left corner
  } else if(!missing(p_value_location) & tolower(p_value)=="yes"){ #if p_value is wanted and p_value_location is not missing
    print("p_value_location: TR-top right; TL- top left; BR- bottom right; BL- bottom left")
  ...
  #making sure only pdf and png is selected for output_format
  output_format <- match.arg(output_format)
  if(missing(output_format)) { output_format<-"pdf"} #default being output_format is pdf
  if (output_format=="pdf"){
    if (missing(width)) width<-8
    if (missing(height)) height<-8
  } else if (output_format=="png"){
    if (missing(width)) width<-200
    if (missing(height)) height<-200
  } 
  
  #setting up the colors
  sample_type_color_t<-sample_type_color
  names(sample_type_color_t) <-sample_types
  if (!missing(dot_sample_type_var_name)){
      #setting up the dot colors
      dot_sample_type_color_t<-dot_sample_type_color
      names(dot_sample_type_color_t) <-dot_sample_types
      
      all_color<-c(sample_type_color_t,dot_sample_type_color_t)
  } else {
    all_color<-sample_type_color_t
  }
  #making sure the coloring and order of the input for the sample_types are going to align with the output graph
  sample_type2<-sort(sample_types)
  
  #only run this when dot_sample_type_var_name is not missing. 
  if (!missing(dot_sample_type_var_name)){
    stopifnot("dot_sample_type_var_name is not contained within the dataframe"= any(grepl(dot_sample_type_var_name, input_table[, 1])))
    
    dot_sample<-input_table[grepl(dot_sample_type_var_name, input_table[, 1]),]
    dot_sample<<-dot_sample[,-1]
    dot_sample<-t(dot_sample)
    input_table<-input_table[!grepl(dot_sample_type_var_name, input_table[, 1]),]
  }
  ...
  if(turn_relative_ab=="yes"){
          input_table4 <- input_table4 %>%
  ...
  #Merge PC Data with MetaData
  newResults <- merge(x = CmdScale, y = sample_type_table, by = "row.names", all.x = TRUE)  
  ...
  #only run this when dot_sample_type_var_name is not missing. 
  if (!missing(dot_sample_type_var_name)){
    newResults<-merge(newResults, dot_sample, by.x="name", by.y=0)
  }
  ...
  p<-ggplot(newResults) + # Graph PC1 and PC2
    xlab(paste0("PC1: ",percentVar[1],"% variance")) + #Label PC1
    ylab(paste0("PC2: ",percentVar[2],"% variance")) + #Label PC2 
    labs(title=NULL, 
         fill="Centroids") +
    ...
    geom_segment(aes(x=PC1.centroid, y=PC2.centroid, xend=PC1, yend=PC2, color= get(sample_type_var_name)))+ 
    geom_point(data=centroids, aes(x=PC1, y=PC2, color= get(sample_type_var_name)), size=0) + 
    geom_label_repel(data = centroids, aes(x=PC1, y=PC2, label=sample_type2, color= get(sample_type_var_name)), size=label_size) +
    scale_color_manual(name=NULL,  
                       values=all_color) +
    theme(panel.background = element_blank(),
          panel.border=element_rect(fill=NA),
  ...
  if (!missing(dot_sample_type_var_name)){
          p<-p+ geom_point(aes(PC1, PC2, color= get(dot_sample_type_var_name)), size=2.5) # Set the size of the points
  } else {
          p<-p+ geom_point(aes(PC1, PC2, color= get(sample_type_var_name)), size=2.5) # Set the size of the points
  }
...
PLS_DA_nonPS_KW <- function(input_table, 
                      ...
                      flip_loading_direction=c("yes","no"),
                      output_name, 
                      output_format=c("pdf", "png"),
                      width, height) {
  ...
  if(missing(flip_loading_direction)) flip_loading_direction<-"no"
  ...
  if (output_format=="pdf"){
        ...
        if (missing(width)) width<-500
        if (missing(height)) height<-500
  } 
  #setting up color
  sample_type_color_t<-sample_type_color
  names(sample_type_color_t) <-sample_types  
  ...
  if (output_format=="pdf"){
          pdf_output1<- paste0(output_name,"_a.pdf")
          pdf(pdf_output1, width=width, height=height) #PLSDA with confidence ellipses
                show(p3)
          dev.off()
  } else if (output_format=="png"){
          png_output1<- paste0(output_name,"_a.png")
  ...
  export<-MyResult.splsda$loadings$X
  assign("PLSDA__loading_significant",export, envir = .GlobalEnv )
  write.csv(export,paste0(output_name,"__loading_significant.csv"))
  ...
  p4<-plotLoadings(MyResult.splsda, 
                   ...
                   comp=1, 
                   ndisplay = top_display_loading, name.var=col_name) ##display top components
  if(flip_loading_direction=="yes"){
    p4$importance<-p4$importance*-1
    p <- ggplot(p4, aes(x = reorder(rownames(p4), importance), y = importance))+
  ...
  if (output_format=="pdf"){
          pdf_output3<- paste0(output_name,"_b.pdf")
          pdf(pdf_output3,width=width*1.5, height=height) #Plot of Loading vectors
               show(p)
          dev.off()
  } else if (output_format=="png"){
          png_output3<- paste0(output_name,"_b.png")
...
Edger_nonPS_KW <- function (input_table, 
                            ...
                            display_all_results_volcano=c("yes","no"), #display all the results in volcano plot (including results that is not significant: FDR not below the cut off)
                            abundance_size_volcano=c("yes","no"), #change size of the dot based on relative abundance
                            volcano_label_bold="",volcano_bold_size=3,volcano_bold_color="black",
                            label_only_volcano, 
                            output_name,
                            legend_onplot=c("yes","no"),
                            plot_title="EgdeR",
  ...
  dgeFull_test<<-dgeFull
  dgeTest_test<<-dgeTest
  #Create Table
  ...
  #renaming the abundance so the sample_type with attached along with abundance
  abundance1<-paste0("abundnace__",sample_types[[1]])
  abundance2<-paste0("abundnace__",sample_types[[2]])
  names(res_all)[names(res_all) == "abundance.1"] <- abundance1
  names(res_all)[names(res_all) == "abundance.2"] <- abundance2
  ...
  if (graph_option=="volcano") {
    ...
    if(!missing(label_only_volcano)){
      p<-p+geom_text_repel(aes(label=ifelse(res$logFC<(-1) & res$adj.P.Val < FDR_cut_off & res$Name  %in% label_only_volcano, as.character(res$Name),'')),box.padding = 0.5,
                           max.overlaps = Inf,force=5,segment.colour="grey",segment.alpha=0.5, 
                           size=ifelse(res$Name %in% volcano_label_bold, volcano_bold_size, 3), #change size on volano_label_bold
                           fontface=ifelse(res$Name %in% volcano_label_bold, 'bold', 'plain'),  #change to bold on volano_label_bold
                           colour=ifelse(res$Name %in% volcano_label_bold, volcano_bold_color, 'black'))+  #change color on volano_label_bold
        geom_text_repel(aes(label=ifelse(res$logFC>1 & res$adj.P.Val < FDR_cut_off  & res$Name  %in% label_only_volcano, as.character(res$Name),'')),box.padding = 0.5,
                        max.overlaps = Inf,force=5,segment.colour="grey",segment.alpha=0.5, 
                        size=ifelse(res$Name %in% volcano_label_bold, volcano_bold_size, 3), #change size on volano_label_bold
                        fontface=ifelse(res$Name %in% volcano_label_bold, 'bold', 'plain'),  #change to bold on volano_label_bold
                        colour=ifelse(res$Name %in% volcano_label_bold, volcano_bold_color, 'black'))  #change color on volano_label_bold
    }else{
      p<-p+geom_text_repel(aes(label=ifelse(res$logFC<(-1) & res$adj.P.Val < FDR_cut_off, as.character(res$Name),'')),force=25,segment.colour="grey",segment.alpha=0.5, 
                           size=ifelse(res$Name %in% volcano_label_bold, volcano_bold_size, 3), #change size on volano_label_bold
                           fontface=ifelse(res$Name %in% volcano_label_bold, 'bold', 'plain'),  #change to bold on volano_label_bold
                           colour=ifelse(res$Name %in% volcano_label_bold, volcano_bold_color, 'black'))+  #change color on volano_label_bold
        geom_text_repel(aes(label=ifelse(res$logFC>1 & res$adj.P.Val < FDR_cut_off , as.character(res$Name),'')),force=25,segment.colour="grey",segment.alpha=0.5, 
                        size=ifelse(res$Name %in% volcano_label_bold, volcano_bold_size, 3), #change size on volano_label_bold
                        fontface=ifelse(res$Name %in% volcano_label_bold, 'bold', 'plain'),  #change to bold on volano_label_bold
                        colour=ifelse(res$Name %in% volcano_label_bold, volcano_bold_color, 'black'))  #change color on volano_label_bold
  ...
  if (legend_onplot=="no") { #if want to hide the legend on the plot, then will save the legend on separate png
    ...
    legend<-as_ggplot(leg)
    if (output_format=="pdf"){
      pdf_output2<-paste0(output_name,"_legend.pdf")
      pdf(pdf_output2, width=width, height=height)
          show(p)
      dev.off()
    } else if (output_format=="png"){
      png_output2<-paste0(output_name,"_legend.png")
  ...
  if (output_format=="pdf"){
    pdf_output<-paste0(output_name,".pdf")
    pdf(pdf_output, width=width, height=height)
    show(p)
    dev.off()
  } else if (output_format=="png"){
    png_output<-paste0(output_name,".png")
    png(png_output, res = 300, width=width, height=height, units='mm')
    show(legend)
    dev.off()
  }
}
...
Edger_multi_compare_nonPS_KW <- function (input_table, 
                            sample_type_var_name, 
                            sample_types=list(), 
                            sample_type_color=list(),
                            compare_sets=list(),
                            ...
                            number_display=10,
                            output_name,
                            plot_title="EdgeR Multi-comparison",
                            legend_onplot=c("yes","no")) {
  ...
  ####preparing compare_sets list
  ####The elements within compare_sets need to have "__and__"". for example, if A and B are the sample_types, then "A__and__B" would be comparing A and B.
  for (i in 1:length(compare_sets)) {
    assign(paste0("compare_sets",i), compare_sets[[i]])
  }  
  #make sure each elements within compare_sets have "__and__"
  compare_sets_check <- grepl("__and__", compare_sets)
  all_true <- all(compare_sets_check)
  stopifnot("compare_sets need to contain element with __and__ to determine the pairs for comparson"= all_true)
  
  #~~export xlsx: export all the results without filtering the nonsignificant results
  wb_all <- createWorkbook()
  #~~export xlsx: export only significant results
  wb_significant <- createWorkbook()
  #~~export xlsx: export only displayed results
  wb_display <- createWorkbook()
  
  test_num<-1
  for (h in 1:length(compare_sets)) {
      name_compare_set<-get(paste0("compare_sets",h))
      compare_sublist <- unlist(strsplit(name_compare_set, "__and__"))
      print("Comparing: ")
      print(compare_sublist)
      
      #~~export xlsx: adding worksheet to the workbook 
      #~~export xlsx: Add a sheet to the workbook
      addWorksheet(wb_all, name_compare_set)
      addWorksheet(wb_significant, name_compare_set)
      addWorksheet(wb_display, name_compare_set)
      
      #setting key for the sample type to compare
      for (i in 1:length(compare_sublist)) { 
        assign(paste0("compare_sublist",i), compare_sublist[i])
        stopifnot("the sample types within compare_sets need to be included in sample_types"= compare_sublist %in% sample_types)
      }
      
      #keeping only sample types we care about 
      input_table3<-input_table2[,(input_table2[1,]) %in%  c(sample_type_var_name, compare_sublist)]
      ...
      #making a compare variable to standardize the input variable for comparsion- make sure the reference is always control
      coldata_SAMPLE$sample_type_var_name<-coldata_SAMPLE[[sample_type_var_name]]
      coldata_SAMPLE<- coldata_SAMPLE %>% dplyr::mutate(compare=case_when((sample_type_var_name==compare_sublist1)~"control", (sample_type_var_name==compare_sublist2)~"treatment"))
      coldata_SAMPLE<-coldata_SAMPLE[!is.na(coldata_SAMPLE$compare),] ###dropping subjects if they are not in the group of comparsion
      coldata_SAMPLE$compare<- as.factor(coldata_SAMPLE$compare) #converting variable to factor
      countdata_SAMPLE<-countdata_SAMPLE[,colnames(countdata_SAMPLE) %in% rownames(coldata_SAMPLE)]#making sure coldata has same subjects and countdata
      ...
      df_export<<-df
      #Get the ColData for Each Comparison
      coldata.1 <- coldata_SAMPLE[coldata_SAMPLE[[sample_type_var_name]]==compare_sublist1,]
      coldata.2 <- coldata_SAMPLE[coldata_SAMPLE[[sample_type_var_name]]==compare_sublist2,]
      #keep Count data only for each comparison
      needed<-which(colnames(df) %in% rownames(coldata.1))    
      df.1 <- df[,needed]
      needed2<-which(colnames(df) %in% rownames(coldata.2))    
      ...
      #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#  
      #Set Names of Results Table
      abundance_1<-paste0("abundance-",compare_sublist1)
      abundance_2<-paste0("abundance-",compare_sublist2)
      
                #check to see if there is any taxa with FDR<FDR_cut_off. If not then will stop the function, because there will be error if it continues as the object will have no rows
                checkifallzero<-resNoFilt[resNoFilt$FDR<FDR_cut_off,]
                if (dim(checkifallzero)[1] == 0) {
                  print(paste0("no Taxa with FDR more than",FDR_cut_off))
                  print("No results are produced since no significant results")
                  #########EXCEL EXPORT
                  #res_all contains all results including not significant results
                  #Convert Resuts table into a data.frame
                  res_all <- as.data.frame(resNoFilt)
                  ...
                  df.2.meanRA.save <- df.2.meanRA[otu.to.save_all]
                  #add the abundnace data for the res dataframe
                  res_all[[abundance_1]] <- df.1.meanRA.save
                  res_all[[abundance_2]] <- df.2.meanRA.save
                  
                  #Create Variable for Color based on Comparison, FDR and LOGFC
                  res_all$col <- ifelse(res_all$logFC>0, "B",
                                        ifelse(res_all$logFC<0, "A","D"))
                  res_all$col_color <- ifelse(res_all$FDR<FDR_cut_off,res_all$col, "D" ) #if result is not significant then it will be D
                  #Export result table
                  writeData(wb_all, sheet = name_compare_set, x = res_all, startCol = 1, startRow = 1, colNames=TRUE, rowNames = TRUE)
                  
                  res_all$abundance <- ifelse(res_all$col=="A", res_all[[abundance_1]], ifelse(res_all$col=="B", res_all[[abundance_2]],0))
                  #export result so can be combine into the plot together
                  assign(paste0("result_all_set",test_num), res_all)
   
                  significant_names<- data.frame(Name = character(0), start = numeric(0)) #empty dataframe given no significant results
                  assign(paste0("significant_names",test_num), significant_names)
                } else { #only run this section if there is results that are signifiant. otherwise skip it
                      resNoFilt[nrow(resNoFilt) + 1,] <- c(0, 0, 0, 0) ###adding this row of 0 so that even if there is no significant top 10 for upregulated or downregulated, the code will still run
                      #Create Table of Down Regulated
                      sigDownReg <- resNoFilt[resNoFilt$FDR<FDR_cut_off,] #keeping only FDR < 0.2
                      #sigDownReg <- resNoFilt
                      sigDownReg <- sigDownReg[sigDownReg$logFC<=0,] ##keeping only the negative logFC
                      sigDownReg <- sigDownReg[order(sigDownReg$logFC),]
                      sigDownReg1 <- sigDownReg[sigDownReg$logFC!=0,] 
                      sigDownReg_top <- sigDownReg1 %>% dplyr::slice(1:number_display) #Subset Top (number_display)
                      #Create Table of Down Regulated
                      sigUpReg <- resNoFilt[resNoFilt$FDR<FDR_cut_off,] #keeping only FDR < 0.2
                      #sigUpReg <- resNoFilt
                      sigUpReg <- sigUpReg[sigUpReg$logFC>=0,] ##keeping only the positive logFC
                      sigUpReg <- sigUpReg[order(sigUpReg$logFC, decreasing=TRUE),]
                      sigUpReg1 <- sigUpReg[sigUpReg$logFC!=0,] 
                      sigUpReg_top <- sigUpReg1 %>% dplyr::slice(1:number_display) #Subset Top (number_display)
                      #Merge Top N results
                      #res contains the top N results we care about
                      res <- rbind(sigDownReg_top,sigUpReg_top)
                      #remove the 0 column which was used as a place holder
                      #res only contains the top N upregulated and downregulated results 
                      res <- res[res$logFC!=0,] 
                      resNoFilt <- resNoFilt[resNoFilt$logFC!=0,] 
                      ...
                      res$abundance.1 <- df.1.meanRA.save
                      res$abundance.2 <- df.2.meanRA.save
                      res <- setNames(cbind(rownames(res), res, row.names = NULL), c("Name","logFC", "lfcSE", "pvalue", "adj.P.Val",abundance_1,abundance_2)) 
                      #Remove Any Data without LOGFC data
                      res <- res[!is.na(res$logFC),]
                      # Reorder Results based on FDR for comparison 1
                      res = res[order(-res$logFC, na.last = TRUE), ]
                      #Create Order
                      res <- res %>% dplyr::mutate(start = 1:n())
                      res$start<- res$start + 100*(test_num-1) #make it so start is not stacking for each comparison 
                      #Convert Important columns to Numeric
                      res$adj.P.Val <-   as.numeric(as.character(res$adj.P.Val))
                      res$logFC <-       as.numeric(as.character(res$logFC))
                      #Replace NA
                      res <- res %>% dplyr::mutate(adj.P.Val = if_else(is.na(adj.P.Val), 0.9, adj.P.Val))
                      ##convert abundance to numeric
                      res[[abundance_1]] <- as.numeric(as.character(res[[abundance_1]]))
                      res[[abundance_2]] <- as.numeric(as.character(res[[abundance_2]]))
                      #Create Variable for Color based on Comparison, FDR and LOGFC
                      res$col <- ifelse(res$logFC>0, "B",
                                        ifelse(res$logFC<0, "A","D"))
                      res$col_color <- ifelse(res$adj.P.Val<FDR_cut_off,res$col, "D" ) #if result is not significant then it will be D
                      res$abundance <- ifelse(res$col=="A", res[[abundance_1]], ifelse(res$col=="B", res[[abundance_2]],0))
                      
                      #significant_names contain all the significant results
                      significant_names<-res %>% select(c("Name", "start"))
                      assign(paste0("significant_names",test_num), significant_names)
                      ...
                      df.2.meanRA.save <- df.2.meanRA[otu.to.save_sign]
                      #add the abundnace data for the res dataframe
                      res_sign[[abundance_1]] <- df.1.meanRA.save
                      res_sign[[abundance_2]] <- df.2.meanRA.save
                      #Export result table
                      writeData(wb_significant, sheet = name_compare_set, x = res_sign, startCol = 1, startRow = 1, colNames=TRUE,  rowNames = TRUE)
                      ...
                      res_all$abundance <- ifelse(res_all$col=="A", res_all[[abundance_1]], ifelse(res_all$col=="B", res_all[[abundance_2]],0))
                      #export result so can be combine into the plot together
                      assign(paste0("result_top_set",test_num), res)
                      assign(paste0("result_all_set",test_num), res_all)
                }
      test_num<-test_num+1
  }
  #~~export xlsx: export all the results without filtering the nonsignificant results
  output_file_all<-paste0(output_name,"_all.xlsx")
  saveWorkbook(wb_all, file = output_file_all, overwrite=TRUE)
  #~~export xlsx: export only significant results
  output_file_sign<-paste0(output_name,"_sign.xlsx")
  saveWorkbook(wb_significant, file = output_file_sign, overwrite=TRUE)

  #check to see if any of the comparison were significant
      #all_objects contains all of the comparison with significant results 
      all_objects<-list()
      test_num<-1
      for (h in 1:length(compare_sets)) {
        all_objects<-c(all_objects,paste0(paste0("significant_names",test_num)))
        test_num<-test_num+1
      }
      # Check if each of the "sampled" dataframes is empty
      empty_sampled_dataframes <- lapply(all_objects, function(obj_name) {
        df <- get(obj_name)
        if (nrow(df) == 0) {
          is_empty <- TRUE} else{
          is_empty <- FALSE
      ...
      # Check if all "sampled" dataframes are empty
      if (all(unlist(empty_sampled_dataframes))==TRUE) {
        print("None of the comparison were significant!!!!!!!!!")
        print("No output pdf made")
        return()
      } 
      
  test_num2<-2
  significant_names_combine<- significant_names1
  for (h in 2:length(compare_sets)) {
    ###significant_names_combine combines all of the significant names
    significant_names_combine<-rbind(significant_names_combine, get(paste0("significant_names",test_num2)))
    test_num2<-test_num2+1 
  }
  
  #figure out the order for display (will take the minimum number for each unique name)
  #keep only unique 
  significant_names_combine_f <- aggregate(start ~ Name, data = significant_names_combine, FUN = min)
  rownames(significant_names_combine_f)<-significant_names_combine_f$Name
  significant_names_combine_f <- significant_names_combine_f %>% select(-c("Name")) #remove the column Name so "start" is the only column left. "start give you the order you want the graph to be display
  ...
  for (h in 1:length(compare_sets)) {
    ...
    #pull back the results with all the results with that particular comparson pair
    res_compare_sets<-get(paste0("result_all_set",test_num3))
    res_compare_sets_temp<-merge(significant_names_combine_f, res_compare_sets, by=0, all.x=T)
    rownames(res_compare_sets_temp)<-res_compare_sets_temp$Row.names
    res_compare_sets_temp <-res_compare_sets_temp %>% select(c("logFC", "logCPM", "PValue", "FDR",abundance_1,abundance_2,"col", "col_color", "start"))
    #export the display result that end up in the final graph
    writeData(wb_display, sheet = name_compare_set, x = res_compare_sets_temp, startCol = 1, startRow = 1, colNames=TRUE,  rowNames = TRUE)
    res_compare_sets_temp$abundance <- ifelse(res_compare_sets_temp$col=="A", res_compare_sets_temp[[abundance_1]], ifelse(res_compare_sets_temp$col=="B", res_compare_sets_temp[[abundance_2]],0))
    res_compare_sets_temp$Name<-rownames(res_compare_sets_temp)

          #=========================================================
          #////////////////////Lollipop PLOT///////////////////////
          #=========================================================
          p<-ggplot(res_compare_sets_temp, aes(y=reorder(Name,-start), x=logFC,fill=col_color,size=abundance)) +
            geom_point(color="black",alpha=0.8,shape=21)+
            geom_segment(data=res_compare_sets_temp,aes(yend=reorder(Name,-start)), xend=(-30), color= "black", linetype = "solid",linewidth=1)+ 
            scale_fill_manual(values=c("B"=sample_type_color_t[[compare_sublist2]],"A"=sample_type_color_t[[compare_sublist1]],"D"="white"))+ 
            scale_size_continuous(name="Relative Abundance",range=c(5, 20))+                
            theme(panel.background = element_blank(),
                  panel.border=element_rect(fill=NA),
                  panel.grid.major.y = element_line(colour = "#EBEBEB",linetype="dashed"),
          ...
          if(test_num3==1){
            #make the myplot list with the first run of the loop
            myplots <- list()  # new empty list
                #if want legend on plot
                if(legend_onplot=="no"){ 
                  p <- p <-theme(legend.position = "none")
                }else {
                  p <-p + theme(legend.background = element_rect(color=NA),
                                legend.key = element_rect(colour = "transparent", fill = "white"))
                }
            myplots[[test_num3]]<- p
          } else{
            myplots[[test_num3]] <- p + remove_y
          }
    
    test_num3<-test_num3+1
  }
  
  ### GGARRANGE ALL PLOTS ### 
  plot.all<-patchwork::wrap_plots(myplots,  ncol=length(myplots), nrow=1) + plot_layout(guides = "collect")  &  
    plot_annotation(title = plot_title, theme= theme(title = element_text(color = "red", face = "bold", size = 24)))
  
  pdf_output=paste0(output_name,"_final_display.pdf")
  pdf(file=pdf_output, width=20+(length(compare_sets))*5, height=12)
      show(plot.all)
  dev.off()        
  
  #~~export xlsx: export only significant results
  output_file_display<-paste0(output_name,"_final_display.xlsx")
  saveWorkbook(wb_display, file = output_file_display, overwrite=TRUE)
  
}

Edger_multi_compare_select_KW <- function (input_excel, 
                                           output_interest_list=list(), #this is where you input the list of output (taxa/metabolite/ko- depends what information the input_excel contain) that you want in the chart
                                           sample_types=list(), 
                                           sample_type_color=list(),
                                           output_name,
                                           legend_onplot=c("yes","no")) {
      ##this funciton uses the output from Edger_multi_compare_nonPS_KW and display only the result you want
      ...
      sheet_names <- excel_sheets(input_excel)
    
      print("this excel sheet contain compare_sets:")
      print(sheet_names)

      df_index <- data.frame(ID = 1:length(output_interest_list), Description = output_interest_list)
      
      sheet_data <- list()
      l<-1
      # Loop through the sheet names and read data
      for (sheet_name in sheet_names) {
        data <- read_excel(input_excel, sheet = sheet_name)
        data<-data[(data$...1 %in% output_interest_list),]
        
        compare_sublist <- unlist(strsplit(sheet_name, "__and__"))
        print("Comparing: ")
        print(compare_sublist)
        ...
        data$abundance <- ifelse(data$col=="A", data[[6]], ifelse(data$col=="B", data[[7]],0))
        data$Name <- data$...1
        
        data<-merge(data, df_index, by.x="Name", by.y="Description", all=TRUE)
        p<-ggplot(data, aes(y=reorder(Name,-ID), x=logFC,fill=col_color,size=abundance)) +
          geom_point(color="black",alpha=0.8,shape=21)+
          geom_segment(data=data,aes(yend=reorder(Name,-ID)), xend=(-30), color= "black", linetype = "solid",linewidth=1)+ 
          scale_fill_manual(values=c("B"=sample_type_color_t[[compare_sublist2]],"A"=sample_type_color_t[[compare_sublist1]],"D"="white"))+ 
          scale_size_continuous(name="Relative Abundance",range=c(5, 20))+                
          theme(panel.background = element_blank(),
                panel.border=element_rect(fill=NA),
        ...
        if(l==1){
          ...
          myplots[[l]] <- p + remove_y
        }
        
        assign(paste0("data",l),data)
        l<-l+1
      }
    
      ### GGARRANGE ALL PLOTS ### 
      plot.all<-patchwork::wrap_plots(myplots,  ncol=length(myplots), nrow=1) + plot_layout(guides = "collect")  &  
        plot_annotation(title = output_name, theme= theme(title = element_text(color = "red", face = "bold", size = 24)))
      
      pdf_output=paste0(output_name,"_select_display.pdf")
      pdf(file=pdf_output, width=20+(length(sheet_names))*5, height=12)
            show(plot.all)
      dev.off()        
...
boxplot_metabolites_KW <- function(input_table, 
                             sample_type_var_name, 
                             sample_types=list(), 
                             sample_type_color=list(), 
                             metabolite_list=list(),
                             highlight_list=list(),
                             relative_ab=c("yes","no"),
                             output_name,
                             new_name_dataframe,
                             new_name_dataframe_oldname,
                             new_name_dataframe_newname) {
  
  #making sure the number of item in sample_types is same as sample_type_color
  stopifnot("sample_types need to have same number of elements as sample_type_color"= length(sample_types)==length(sample_type_color))
  stopifnot("sample_type_var_name needs to exist as column name of input_table" = sample_type_var_name %in% colnames(input_table))
  
  if(!missing(new_name_dataframe)){ #if there is a dataframe with updated names
    indices <- match(metabolite_list, new_name_dataframe[[new_name_dataframe_newname]])
    metabolite_list <- new_name_dataframe[[new_name_dataframe_oldname]][indices]
    metabolite_list_new <- new_name_dataframe[[new_name_dataframe_newname]][indices]
    if(!missing(highlight_list)) {
      indices2 <- match(highlight_list, new_name_dataframe[[new_name_dataframe_newname]])
      highlight_list <- new_name_dataframe[[new_name_dataframe_oldname]][indices2]
      highlight_list_new <- new_name_dataframe[[new_name_dataframe_newname]][indices]
    }
  } 
  stopifnot("the list of metabolites input in 'metabolite_list' needs to exist as column name of input_table" = metabolite_list %in% colnames(input_table))
  if(!missing(highlight_list)) {stopifnot("the list of metabolites input in 'hightlight_list' needs to exist as column name of input_table" = highlight_list %in% colnames(input_table))}
  ...
  #default is absolute value, rather than relatie abundance
  if(missing(relative_ab)) relative_ab<-"no"
  relative_ab<-match.arg(relative_ab)

  if(relative_ab=="yes"){
    #remove the first column which is the identifier so we have just a numeric dataframe so relative abundance can be calculated
    input_table_temp<-t(input_table)
    input_table_temp<-input_table_temp[-1,]
    input_table_temp<-data.frame(input_table_temp)
    
    input_table_tempsedf<<-input_table_temp
    
    input_table_temp[] <- apply(input_table_temp, 2, as.numeric)
    #calculate relative abundnace
    df <-
      input_table_temp %>% 
      rownames_to_column('gs') %>%
      dplyr::group_by(gs) %>% 
      ...
      dplyr::mutate_if(is.numeric, funs(./sum(.))) %>%
      column_to_rownames('gs')
    input_table2_temp<-t(df)
    input_table2<-cbind(input_table[,1],input_table2_temp)
    #make sure the first column look as the initial input_table
    colnames(input_table2)[1] <- sample_type_var_name
    #turning it back to a dataframe
    input_table2<-data.frame(input_table2)
    input_table2[,-1] <- apply(input_table2[,-1], 2, as.numeric)
  }else {
    input_table2<-input_table
  }

  input_table_graph <- input_table2 %>%
    pivot_longer(cols = -sample_type_var_name, names_to = "Metabolites", values_to = "Value")
  myComparisons<-as.list(as.data.frame(combn(unique(unlist(sample_types)), 2))) 
  
  input_table_graph<-input_table_graph[input_table_graph[[sample_type_var_name]] %in% sample_types,]
  
  if(!missing(new_name_dataframe)){
    input_table_graph<- merge(input_table_graph,new_name_dataframe, by.x="Metabolites", by.y=new_name_dataframe_oldname, all.x=T)
    input_table_graph<-input_table_graph %>% select(-c("Metabolites"))
    input_table_graph<- input_table_graph %>% dplyr::rename("Metabolites"=new_name_dataframe_newname)
    metabolite_list<-metabolite_list_new #updating the metabolite list based on the new name
    if(!missing(highlight_list)) { highlight_list<-highlight_list_new} #updating the highlight list based on the new name
  }

  num_metabolite_list<-length(metabolite_list)
  input_table_graph<- input_table_graph[input_table_graph$Metabolites %in% metabolite_list,] 
  desired_order <- metabolite_list
  # Reorder the levels of Metabolites based on the desired order
  input_table_graph$Metabolites <- factor(input_table_graph$Metabolites, levels = desired_order)

  q<-ggplot(input_table_graph, aes(x=get(sample_type_var_name), y=Value, fill=get(sample_type_var_name))) + 
    geom_boxplot(width = 0.3)+
    geom_signif(comparisons = myComparisons, 
                map_signif_level = F, textsize = 4, 
                vjust = -0.5, step_increase = 0.05, 
                test = "wilcox.test", tip_length = 0.01) +
    facet_wrap(~Metabolites, ncol = 1, scales = "free_x", dir = "h")+
    coord_flip()+
    ylab("Relative Abundnace") +  # Change the x-axis label
    xlab("Sample type") +  # Change the y-axis label
    scale_y_continuous() +
    scale_fill_manual(values = sample_type_color)+
    theme(panel.background = element_blank(),
          panel.border=element_rect(fill=NA),
          panel.grid.minor = element_blank(),strip.background=element_blank(),
          plot.title=element_text(face="bold",hjust=0.5, size = 28), 
  ...
  for(i in gp.num) {
    if(!is.null(gp$grobs[[i]]$grobs[[1]]$children[[2]]$children[[1]]) &&
       gp$grobs[[i]]$grobs[[1]]$children[[2]]$children[[1]]$label %in% highlight_list) {
  ...
  pdf_output=paste0(output_name,".pdf")
  pdf(file=pdf_output, width=9, height=2*num_metabolite_list)
        plot(gp)
  dev.off()    
}   
...
heatmap_nonPS_KW <- function(input_table, 
                             sample_type_var_name, 
                             sample_types=list(), 
                             sample_type_color=list(),
                             top_N_rows,
                             dendogram_column=c(TRUE,FALSE), 
                             dendogram_row=c(TRUE,FALSE), 
                             ...
                             dendrogram_display=c("none","column","row","both"),
                             width=20, height=20,
                             xaxis_font=2,
                             yaxis_font=2,
                             xaxis_margin=10, 
                             ...
                             legend_size=2,
                             log_value=c("no","yes"),
                             color_palette_set=list(),
                             color_palette_num=200,
                             legend_position=c("topleft","bottomleft","topright","bottomright"),
                             output_name) {
  #print instruction for the function
  print("This function requires your input_table to be a dataframe with subject ID as column name")
  print("First row of the input_table is for classifying the subjects into different sample types")
  ...
  if(missing(dendogram_column))  
    dendogram_column<-TRUE  #default being TRUE for dendogram column, unless specified
  if(missing(dendogram_row))  
    dendogram_row<-TRUE  #default being TRUE for dendogram column, unless specified

  if(missing(color_palette_set))
    color_palette_set<-c("navyblue","white","firebrick3")
  
  log_value<-match.arg(log_value)
  if(missing(log_value)) #default being no for log value
    log_value<-"no"
  
  if(missing(dist_method))
    dist_method<-"euclidean" #default being euclidean
  if(missing(hclust_method))
    hclust_method<-"ward.D" #default being ward.D
  if(missing(dendrogram_display))
    dendrogram_display<-"both" #default being both
  if(missing(legend_position))
    legend_position<-"bottomleft" #default being bottomleft
  ...
  #create a matrix with just the input value 
  abundance_data<-data.frame(input_table3)[-1, -1]
  colnames(abundance_data)<- colnames(input_table3[,-1])
  rownames(abundance_data)<-input_table3[-1,1]
  
  abundance_data[] <- lapply(abundance_data, as.numeric)
  #############################  
  #keeping only the top N rows#
  #############################
  if(!missing(top_N_rows) & is.numeric(top_N_rows)){ #if top_N_rows is not missing
    row_means <- rowMeans(abundance_data)
    # Create an order vector to sort the rows by mean in descending order
    sorted_indices <- order(row_means, decreasing = TRUE)
    
    # Define the number of top rows you want to keep
    top_rows_count <- top_N_rows  # You can change this to keep a different number of rows
    
    # Select the top rows with the highest mean values
    abundance_data <- abundance_data[sorted_indices[1:top_rows_count], ]

  } else if(!missing(top_N_rows) & is.numeric(top_N_rows)){ 
        stop("top_N_rows must be a numeric value")
  } 
  abundance_data2<-as.matrix(abundance_data)
  if(log_value=="yes"){
    abundance_data2<-log(abundance_data2*100+1, base = 10)
  }
  abundance_data2<<-abundance_data2
  
  ##########################
  #Calculating the distance#
  ##########################
  column_distances <- stats::dist(t(abundance_data2), method = dist_method)
  column_hclust <- stats::hclust(column_distances, method = hclust_method)
  row_distances <- stats::dist(abundance_data2, method = dist_method)
  row_hclust <- stats::hclust(row_distances, method = hclust_method)

  #by default column is the sample
  if (dendogram_column==TRUE){
        Col_option<-as.dendrogram(column_hclust)
  } else {Col_option<-NA}
  
  if (dendogram_row==TRUE){
        Row_option<-as.dendrogram(row_hclust)
  } else {Row_option<-NA}
  
  #setting the color
  sample_type_table$Color <- sample_type_color_t[sample_type_table[[sample_type_var_name]]]
  
  #determine the margin based whether the dendrogram is displayed or not
  if (dendrogram_display=="both"){
       top_margin<-2
       left_margin<-2
  } else if (dendrogram_display=="column"){
    top_margin<-0.1
    left_margin<-2    
  } else if (dendrogram_display=="row"){
    top_margin<-2
    left_margin<-0.1    
  } else if (dendrogram_display=="none"){
    top_margin<-0.1
    left_margin<-0.1
  }
  
  pdf_output=paste0(output_name,"_heatmap.pdf")
  pdf(file=pdf_output, width=width, height=height)
          heatmap.2(as.matrix(abundance_data2),                 # Data matrix
                          Colv = Col_option,         # Hierarchical clustering for samples. it will reorder the samples
                          Rowv = Row_option,
                          ColSideColors=sample_type_table$Color,
                          dendrogram = dendrogram_display,            # Display dendrogram
                          ...
                          cexCol  =  xaxis_font,
                          cexRow  =  yaxis_font,
                          margins = c(xaxis_margin, yaxis_margin),  # Adjust margins (top, bottom) for row labels
                          col = colorRampPalette(color_palette_set)(color_palette_num),
                          main = plot_title,
                    )  
          legend(legend_position, title = sample_type_var_name,legend=sample_types, 
                 fill=sample_type_color, cex=legend_size, box.lty=0)
  dev.off()
...
beta_div_removal_testing_KW <- function(contaminant_level_df,
  ...
  for (i in colnames(contaminant_level_df)){
    ...
    #remove samples if all otu is zero
    sample_sums <- colSums(otu_table(phyloseq_t2))
    zero_samples <- names(sample_sums[sample_sums == 0])
    phyloseq_t2 <- prune_samples(!(sample_names(phyloseq_t2) %in% zero_samples), phyloseq_t2)
    ...
    phyloseq_t3SFSDFSD<<-phyloseq_t3
    #Create Distance Matrix with Bray
    if (beta_method=="bray"){
      #Create Distance Matrix with Bray
...
Dickson_graph6B_KW <- function(contaminant_level_df,
  ...
  for (i in colnames(contaminant_level_df)){
    ...
    vegdist_ffwefwef<<-vegdist
    phyloseq_t3dsadada<<-phyloseq_t3
...
###########display_table is a function which display csv into a table on png. the output png will have the name as the csv file
display_table <- function(csv_file_name, title) {
  csv_file<- paste(csv_file_name,".", "csv", sep = "")
  display_data<-read.csv(csv_file)  
  if (ncol(display_data)>100 | nrow(display_data)>100) {
    stop("This table is big. please limit the table size to less than 100 rows/columns")
  }
  #determine the width and height  
  w <- ncol(display_data)*100 ###determine the height and width of the png based on the size of the table
  h <-nrow(display_data)*24 
  df <- data.table(display_data)
  img_Name <- paste(csv_file_name,".", "png", sep = "")
  png(img_Name, width=w, height=h)
  p<-tableGrob(df, rows = NULL)
  grid.arrange(top=textGrob(title, gp=gpar(fontsize=30,font=8)),p)
  dev.off()
}


###########PlotQEA.MetSet_KW is altered metaboanalyst function 
PlotQEA.MetSet_KW <- function (mSetObj = NA, path_id, path_name, rownum,
                               sample_types=list(), 
                               sample_type_color=list(),
                               path_complete_set) {
        ...
        none_color_t<-"white"
        names(none_color_t) <- "none"
        all_color<-c(sample_type_color_t, none_color_t)
        
        select_pathway<-as.data.frame(path_complete_set[path_id]) #look at which metabolites is within this pathway
        select_pathway2<- tibble::rownames_to_column(select_pathway, "metabolites")
        hit.cmpds<-as.vector(select_pathway2[path_id])
        num <- length(hit.cmpds[[1]])
        len <- length(mSet$dataSet$cls)
        ###figure keeping only the hit metabolite 
        ...
        path.data <- mSetObj$dataSet$norm[, hit.inx] #removed if there is no KEGG ID
        colnames(path.data) <- nm.map$kegg[kegg.inx[hit.inx]]
        exist_cmpd<-NULL
        ##keeping only the metabolites which exist in the dataset 
        for (i in 1:num) {
          cmpd <- hit.cmpds[[1]][i]
          if(cmpd %in% colnames(path.data))
          {
            print(cmpd)
            exist_cmpd <- c(exist_cmpd,cmpd)
          }
        }  
        hit_num <- length(exist_cmpd)
        conc.vec <- lbl.vec <- cls.vec <- NULL
        for (i in 1:hit_num) {
          cmpd <- exist_cmpd[i]
          print(cmpd)
          conc.vec <- c(conc.vec, path.data[,cmpd])
          cls.vec <- c(cls.vec, as.character(mSet$dataSet$cls))
          lbl.vec <- c(lbl.vec, rep(cmpd, len))
        }
        cls.vec <- as.factor(cls.vec)
        lbl.vec <- factor(lbl.vec, levels = unique(lbl.vec))
        num <- length(hit.cmpds)
        boxdata <- data.frame(Feature = lbl.vec, Abundance = conc.vec, 
                              Class = cls.vec)
        #Compare two groups (non-parametric) Wilcoxon
        significance<-compare_means(Abundance ~ Class, data = boxdata, 
                                    group.by = "Feature")
        ###this section try to figure out which Class has higher mean among the features and determine whether those are statistically significant
        test<- boxdata %>% dplyr::group_by(Feature,Class) %>% dplyr::summarize(mean(Abundance)) %>% 
                        dplyr::rename(Abundance='mean(Abundance)', Higher_Class=Class) %>%
                        dplyr::group_by(Feature) %>% dplyr::mutate(Flag = case_when(Abundance == max(Abundance) ~ 1,TRUE~ 0)) %>%
                        dplyr::filter(Flag ==1) %>%
                        dplyr::select(c("Feature","Higher_Class")) ## this contains the class with higher mean abundance among the different features
        
        test2<- significance %>% dplyr::select(c("Feature","p.signif"))
        
        test3 <- merge(boxdata, test, by="Feature")
        test4 <- merge(test3, test2, by="Feature")
        
        test4$Higher_Class[test4$p.signif == "ns"] <- NA 
        test4$Higher_Class<-as.character(test4$Higher_Class)
        test4$Higher_Class<- test4$Higher_Class %>% replace_na('none')
        test4$Higher_Class<-as.factor(test4$Higher_Class)
        test4$significant_class<-test4$Class
        test4$significant_class[test4$p.signif == "ns"] <- NA 
        test4$significant_class<-as.character(test4$significant_class)
        test4$significant_class<- test4$significant_class %>% replace_na('none')
        test4$significant_class<-as.factor(test4$significant_class)
        new_boxdata<- test4 %>% dplyr::select(-c("p.signif"))
        
        col<-unique(new_boxdata$Class)
        col<-as.character(col)
        col<-append(col,"none")
        col<-as.factor(col)
        #replace KEGG ID with metabolite name
        mergedData <- merge(new_boxdata, nm.map, by.x=c("Feature"), by.y=c("kegg"))
        
        exceltable_temp <- mergedData %>% dplyr::select(c("Feature", "Higher_Class", "query"))
        exceltable_temp <- exceltable_temp %>% dplyr::rename(metabolite=query)
        exceltable_temp <- unique(exceltable_temp)
        
        exceltable_temp2 <- significance %>% dplyr::select(c("Feature", "p.signif", "p", "p.adj", "method"))
        
        exceltable_merged <- merge(exceltable_temp, exceltable_temp2, by="Feature")
        
        exceltable_merged <- exceltable_merged %>% select(metabolite, everything())
        exceltable_merged <- exceltable_merged %>% dplyr::rename(KEGG_ID=Feature)
        exceltable_merged$" "  <- " "
        exceltable_merged$index <- 1:nrow(exceltable_merged)
        
        mergedData <- mergedData %>% dplyr::select(-c("hmdb", "hmdbid", "Feature")) ###keeping only the original skeleton name and CID(pubchem ID)
        mergedData <- mergedData %>% dplyr::rename(Feature=query) ###renaming the variables
        
        exceltable_merged2<- mergedData %>% dplyr::select(c("Abundance", "Class", "Feature"))
        exceltable_merged2 <- exceltable_merged2 %>% dplyr::rename(Abundance.=Abundance, Class.=Class, Feature.=Feature)
        exceltable_merged2$index <- 1:nrow(exceltable_merged2)
        
        exceltable <- merge(exceltable_merged2, exceltable_merged, by="index", all=T)
        
        exceltable <- exceltable %>% relocate(c("Abundance.", "Class.", "Feature."), .after = last_col())
        exceltable <- exceltable %>% select(-c("index") )
        
        exceltable <- data.frame(lapply(exceltable, as.character), stringsAsFactors=FALSE)
        
        exceltable[is.na(exceltable)] = " "
        path_name_shortened <- paste0(substr(path_name,1,30-nchar(word(path_name,-1))),paste0(".",word(path_name,-1)))
        path_name_shortened <- gsub(" ", ".", path_name_shortened)
        
        addWorksheet(wb=wbook, sheetName=path_name_shortened)
        writeData(wb=wbook, sheet=path_name_shortened, x=exceltable)
        
        print(path_name_shortened)
        print(path_name)
        
        writeFormula(wbook, "pathway",
                     startRow = rownum,
                     x = makeHyperlinkString(
                       sheet = path_name_shortened, row = 1, col = 1,
                       text = path_name
                     )
        )
        ...
        ##########################################################################################################
        p<-ggboxplot(mergedData, x = "Feature", y = "Abundance", color = "Class", fill="significant_class",  
                     palette = sample_type_color_t) +scale_fill_manual(values= all_color) + guides(fill="none")
        p + rotate_x_text(angle = 45) + theme(axis.text = element_text(size = 7))  + aes(stringr::str_wrap(Feature, 15),Abundance) + 
          stat_compare_means(aes(group = Class), label = "p.signif" , size=6, hide.ns = TRUE) +
          ggtitle(str_to_title(path_name)) + theme(plot.title = element_text(lineheight=3, face="bold", color="black", size=14)) +
          xlab(NULL) + ylab("Scaled Intensities")
        imgName <- paste(path_id, "dpi300.png", sep = "")
        ggsave(imgName, device = "png")
        mSetObj$imgSet$current.img <- imgName
        return(mSetObj)
}

###CalculateQeaScore_KW is an edited version of CalculateQeaScore from Metaboanalyst. the function was edited to add Q statistics of the enrichment portion to the pathway table.
###The QEA and Pathway Analysis global test produce different results. QEA was using KEGG compound name while Pathway Analysis was using only KEGG ID
###https://www.omicsforum.ca/t/enrichment-analysis-and-pathway-analysis-provide-different-globaltest-results/796
CalculateQeaScore_KW<- function (mSetObj = NA) {
        nm.map <- GetFinalNameMap(mSetObj)
        valid.inx <- !(is.na(nm.map$kegg) | duplicated(nm.map$kegg))
        nm.map <- nm.map[valid.inx, ]
        orig.nms <- nm.map$query
        ...
        destfile <- paste("hsa", ".qs", sep = "")
        my.qs <- paste("https://www.metaboanalyst.ca/resources/libs/kegg/metpa/", 
                       destfile, sep = "")
        if (!file.exists(destfile)) {
          curl::curl_download(my.qs, destfile)
          #download.file(my.qs, destfile) ### the qs file download using this line is corrupted for some reaosn 
        }
        current.kegglib<-qs::qread("hsa.qs")
        current.mset <- current.kegglib$mset.list
        uniq.count <- current.kegglib$uniq.count
        hits <- lapply(current.mset, function(x) {
          x[x %in% colnames(path.data)]
        })
        hit.inx <- unlist(lapply(hits, function(x) {
          length(x)
        }), use.names = FALSE) > 0
        hits <- hits[hit.inx]
        if (length(hits) == 0) {
        ...
        imp.vec <- mapply(function(x, y) {
          sum(x[y])
        }, imp.list, hits)
        set.num <- unlist(lapply(current.mset[hit.inx], length), 
                          use.names = FALSE)
        dat.in <- list(cls = mSetObj$dataSet$cls, data = path.data, 
                       subsets = hits)  
        mSetObj$msgSet$rich.msg <- "The selected pathway enrichment analysis method is \\textbf{Globaltest}."
        gt.obj <- globaltest::gt(dat.in$cls, dat.in$data, 
                                 subsets = dat.in$subsets)
        gt.res <- globaltest::result(gt.obj)
        match.num <- gt.res$`#Cov`
        raw.p <- gt.res$`p-value`
        log.p <- -log10(raw.p)
        q_stat<- gt.res$Statistic
        q_expect<- gt.res$Expected
        holm.p <- p.adjust(raw.p, "holm")
        fdr.p <- p.adjust(raw.p, "fdr")
        res.mat <- cbind(set.num, match.num, q_stat, q_expect, raw.p, holm.p, 
                         fdr.p, log.p, imp.vec)
        rownames(res.mat) <- rownames(gt.res)
        colnames(res.mat) <- c("Total Cmpd", "Hits", "Statistic Q", "Expected Q" , "Raw p",  
                               "Holm adjust", "FDR", "neg log10(p)", "Impact")
        res.mat <- res.mat[!is.na(res.mat[, 7]), , drop = FALSE]
        ord.inx <- order(res.mat[, 5], -res.mat[, 9])
...
normalize_metaboanalyst_KW <- function(input_table){
  input_table_file<-paste0(paste0(current_directory,"/"),input_table)
  mSet<<-InitDataObjects("conc", "msetqea", FALSE)
  mSet<<-Read.TextData(mSet, input_table_file, "colu", "disc")
  mSet<<-CrossReferencing(mSet, "name")
  mSet<<-CreateMappingResultTable(mSet)
  mSet<<-SanityCheckData(mSet)
  mSet<<-ReplaceMin(mSet)
  mSet<<-PreparePrenormData(mSet)
  mSet<<-Normalization(mSet, rowNorm=rowNorm, transNorm=transNorm, scaleNorm=scaleNorm, ratio=ratio, ratioNum=ratioNum)
  #metabolites distribution 
  norm_M_output<-paste0(output_suffix,"_norm_mets")
  mSet<<-PlotNormSummary(mSet, norm_M_output, "png", 72, width=NA)
  #sample distribution
  norm_S_output<-paste0(output_suffix,"_norm_subject")
  mSet<<-PlotSampleNormSummary(mSet, norm_S_output, "png", 72, width=NA)
}


pathway_anaylsis_metaboanalyst_KW <- function(input_table,
                                              sample_types=list(), 
                                              sample_type_color=list(),                             
                                              rowNorm="MedianNorm", #Normalization options 
                                              transNorm="LogNorm", #Normalization options
                                              scaleNorm="MeanCenter", #Normalization options
                                              ratioNum=20, #Normalization options
                                              ratio=FALSE, #Normalization options
                                              path_libname="hsa", #default being human library
                                              output_suffix){
        #load the current directory and add to input_table so it can get the csv file
        current_directory <- getwd()
        input_table_file<-paste0(paste0(current_directory,"/"),input_table)
        
        #Create a folder for all the result from this pathway_analysis_metaboanalyst_KW 
        folder_name <- paste0("pathway_",output_suffix)
        ...
        none_color_t<-"white"
        names(none_color_t) <- "none"
        all_color<-c(sample_type_color, none_color_t)
        
        for (i in 1:length(sample_types)) {
          assign(paste0("sample_types",i), sample_types[[i]])
          assign(paste0("sample_type_color",i), sample_type_color[[i]])
        }  
        # Basic statistics utilizing metaboanalyst package
        # InitDataObjects setting up dataSet Object to utilize Metaboanalyst functions
        mSet<<-InitDataObjects("conc", "msetqea", FALSE)
        mSet<<-Read.TextData(mSet, input_table_file, "colu", "disc")
        mSet<<-CrossReferencing(mSet, "name")
        mSet<<-CreateMappingResultTable(mSet)
        ...
        norm_S_output<-paste0(output_suffix,"_norm_subject")
        mSet<<-PlotSampleNormSummary(mSet, norm_S_output, "png", 72, width=NA)
        mSet<<-SetKEGG.PathLib(mSet, path_libname, "current")
        mSet<<-SetMetabolomeFilter(mSet, F);
        mSet<<-CalculateQeaScore_KW(mSet)
        
        #renaming the output file names
        pathway_excel<-paste0(paste0("pathway_results_",output_suffix),".csv")
        file.rename("pathway_results.csv", pathway_excel)
        #QEA bar plot 
        pathway_results_temp<- read.csv(pathway_excel)
        display_table(paste0("pathway_results_",output_suffix),"Pathway Analysis")
        
        ###################################################################################################
        ####generating excel with sheet to show the pathway results with additional sheets for each pathway 
        # Write the first data set in a new workbook
        wbook<<-createWorkbook()
        addWorksheet(wb=wbook, sheetName="pathway")
        writeData(wb=wbook, sheet="pathway", x=pathway_results_temp)
        ###################################################################################################
        pathway_results_temp$ratio<-pathway_results_temp$'Statistic.Q'/pathway_results_temp$'Expected.Q'
        pathway_results_temp_top25<-pathway_results_temp %>% top_n(ratio, n= 25) #keep the top 25 pathway set with highest enrichment ratio 
        pathway_ggplot<-paste0(paste0("qea_",output_suffix),"_dpi300.png")
        pathway_results_temp_top25 %>%
          mutate(X=fct_reorder(X, ratio)) %>%
          ggplot(aes(x=ratio,y=X, fill=FDR)) +
          geom_bar(stat="identity", alpha=.6, width=.4) +scale_fill_gradient(low="red",high="yellow") +
          labs( x="enrichment ratio", y="pathway", title="Quantitative Enrichment Analysis") +
          theme_bw()
        ggsave(pathway_ggplot,  dpi = 300, width = 6, height = 10)
        ########displaying the difference between groups for the quantitative enrichment analysis 
        msea_qea_result<-read.csv(pathway_excel)#getting the file back on R
        enrichment_meta_group <-as.data.frame(msea_qea_result$X)
        enrichment_meta_group$index <- 1:nrow(enrichment_meta_group)
        ###add in the hsa# for each pathway 
        current.kegglib<-qs::qread("hsa.qs")
        current.mset <- as.data.frame(current.kegglib$path.ids)
        path_complete_set<-current.kegglib$mset.list
        pathway_name<- tibble::rownames_to_column(current.mset, "msea_qea_result$X")
        enrichment_meta_group <- merge(enrichment_meta_group,pathway_name,by="msea_qea_result$X")
        enrichment_meta_group<-enrichment_meta_group[order(enrichment_meta_group$index),]
        max_num<-nrow(enrichment_meta_group)
        enrichment_meta_group <-head(enrichment_meta_group,n=max_num)##keeping the top N group
        j<-1
        t<-2
        for (i in enrichment_meta_group$`current.kegglib$path.ids`) {
          print(i)
          k<-enrichment_meta_group$`msea_qea_result$X`[j]
          mSet<<-PlotQEA.MetSet_KW(mSet, i, k, t,
                                  sample_types, 
                                  sample_type_color,
                                  path_complete_set)
          imgName <- paste(i, "dpi300.png", sep = "")
          file.rename(imgName,paste(output_suffix,imgName,sep = ""))
          j<-j+1
          t<-t+1
        }
        pathway_excel_all<-paste0(paste0("pathway_results_",output_suffix),"_all.xlsx")
        saveWorkbook(wb = wbook, file = pathway_excel_all, overwrite = TRUE) #### saving the workbook with all the pathway information
        #Pathway graph 
        mSet$analSet$qea.mat <<- mSet$analSet$qea.mat[, c("Total Cmpd", "Hits", "Raw p", "neg log10(p)", "Holm adjust", "FDR", "Impact", "Statistic Q", "Expected Q")]
        pathway_enrichment<-as.data.frame(mSet$analSet$qea.mat)
        pathway_enrichment <- pathway_enrichment %>%  dplyr::rename(logp="neg log10(p)") ###renaming the variables
        pathway_enrichment<-pathway_enrichment[order(pathway_enrichment$logp,decreasing=TRUE),]
        pathway_enrichment$top_logp <- 1:nrow(pathway_enrichment) #get a ranking based on neg log p value
        pathway_enrichment<-pathway_enrichment[order(pathway_enrichment$Impact,decreasing=TRUE),]
        pathway_enrichment$top_Impact <- 1:nrow(pathway_enrichment) # get a ranking based on pathway impact 
        pathway_enrichment<-base::transform(pathway_enrichment, ranking = pmin(top_logp, top_Impact)) #take the best rank on Impact and neg log p
        pathway_enrichment<-pathway_enrichment[order(pathway_enrichment$ranking),] 
        pathway_enrichment$ranking2 <- 1:nrow(pathway_enrichment)
        pathway_enrichment<-pathway_enrichment %>%  mutate(name = ifelse(ranking2<=10, rownames(pathway_enrichment), NA)) #only top 10 metabolite get label
        pathway_enrichment %>%
          ggplot(aes(x=Impact, y=logp, color=logp)) +
          scale_colour_gradientn(colours = c("yellow","orange", "red")) + 
          geom_point(aes(size=Impact))+scale_size_continuous(range = c(2,10)) +
          labs( x="Pathway Impact", y="-log10(p)", title="Pathway Analysis") +
          geom_text_repel(aes(label = name), size=5, 
                          segment.size=1, min.segment.length = unit(0, 'lines'), segment.color="grey",
                          box.padding = 0.4, max.overlaps = 10,
                          nudge_y = 0.2, nudge_x = 0.05,colour = "black") + 
          theme_bw()
        
        orignal_pathway_output<-paste0(paste0("pathway_",output_suffix),"_org.png")
        ggsave(orignal_pathway_output,  dpi = 300, width = 10, height = 10)
        ##adding catergories of the pathways 
        pathway_description <- read.csv("C:/Users/kendr/OneDrive/Desktop/research/raw_data/KEGG_pathway_grouping.csv")
        rownames(pathway_description) <- pathway_description$pathway_name 
        pathway_description<- subset(pathway_description, select = - c(pathway_name))
        pathway_enrichment <- merge(pathway_enrichment, pathway_description, by=0, all=F)
        pathway_enrichment %>%
          ggplot(aes(x=Impact, y=logp, color=pathway_type)) +
          scale_color_manual(values = c("Amino acid metabolism" = "red",
                                        "Metabolism of other amino acids" = "red3",
                                        "Metabolism of cofactors and vitamins" = "yellow",
                                        "Carbohydrate metabolism" = "green4", 
                                        "Lipid metabolism" = "blue", 
                                        "Nucleotide metabolism" = "goldenrod",
                                        "Biosynthesis of other secondary metabolites" = "hotpink",
                                        "Glycan biosynthesis and metabolism" = "cadetblue2",
                                        "Metabolism of terpenoids and polyketides" = "lightseagreen",
                                        "Energy metabolism" = "purple",
                                        "Genetic Information Translation" = "black",
                                        "Xenobiotics biodegradation and metabolism" = "grey",
                                        "Signal transduction" = "ivory2")) +
          ...
          geom_text_repel(aes(label = name), size=5, 
                          ...
                          box.padding = 0.4, max.overlaps = 10,
                          nudge_y = 0.2, nudge_x = 0.05,colour = "black") + 
          labs(size="Impact", color="Pathway group") +
          theme_bw()
        bytype_pathway_output<-paste0(paste0("pathway_",output_suffix),"_groups.png")
        ggsave(bytype_pathway_output,  dpi = 300, width = 10, height = 10)
